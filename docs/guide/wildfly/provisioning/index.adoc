...
This chapter describes the WildFly provisioning plug-in which feature-packs of WildFly-based distributions reference from their *feature-pack.xml* files.
...
WildFly provisioning plug-in is then responsible for:

** <<config-gen,generating the configuration file(s) based on the installed package set (possibly customized by the user)>>;

** <<module-jars,building "thin" servers (i.e. servers with module.xml files reference module JARs as Maven artifacts) or copying module JARs into the target installation>>;
